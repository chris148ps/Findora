...
public actor LocalModelManager {
    ...
    public func install(
        ...
        guard let descriptor = descriptor(id: modelID) else {
            throw PrivateDocSearchError.processFailed("Unbekanntes Modell: \(modelID)")
        }
        guard profile.compatibility(for: descriptor) != .incompatible else {
            throw PrivateDocSearchError.processFailed("Das Modell ist mit diesem Mac nicht kompatibel.")
        }
        guard profile.availableStorageBytes >= descriptor.downloadSizeBytes * 2 else {
            throw PrivateDocSearchError.processFailed("Nicht genügend freier Speicher für sicheren Download und Installation.")
        }
        ...
        do {
            for index in startIndex..<descriptor.files.count {
                ...
                guard file.downloadURL.scheme == "https",
                    ...
                    throw PrivateDocSearchError.processFailed("Nicht freigegebene Modellquelle.")
                }
                ...
                guard (200...299).contains(http.statusCode),
                    ...
                    throw PrivateDocSearchError.processFailed("Modelldownload wurde von einer nicht freigegebenen Quelle beantwortet.")
                }
                ...
                guard size == file.sizeBytes else {
                    throw PrivateDocSearchError.processFailed("Unerwartete Dateigröße für \(file.relativePath).")
                }
                guard try hasher.hash(fileAt: result.file) == file.checksumSHA256.lowercased() else {
                    throw PrivateDocSearchError.processFailed("Prüfsumme ist falsch: \(file.relativePath)")
                }
    ...
    public func activate(modelID: String) throws {
        guard let descriptor = descriptor(id: modelID),
            ...
            throw PrivateDocSearchError.processFailed("Das Modell ist nicht vollständig installiert.")
        }
    ...
    private func validateModelDirectory(
        ...
        guard validateInstalledFiles(descriptor, directory: directory) else {
            throw PrivateDocSearchError.processFailed("Modellinstallation ist unvollständig.")
        }
        ...
        guard !weightFiles.isEmpty else {
            throw PrivateDocSearchError.processFailed("Modell enthält keine SafeTensors-Gewichte.")
        }
    }
