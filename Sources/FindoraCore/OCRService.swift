...
public struct OCRTemporaryFileCleaner: Sendable {
    ...
    public func removeAbandonedFiles(
        ...
        guard let enumerator = FileManager.default.enumerator(
            ...
            throw FindoraError.permissionDenied(root.path)
        }
        ...
        while let url = enumerator.nextObject() as? URL {
            ...
            guard values?.isRegularFile == true,
                  url.lastPathComponent.hasPrefix(".findora-ocr-"),
...
public actor OCRmyPDFProcessor: OCRProvider {
    ...
    public func process(
        ...
        guard configuration.isEnabled else { throw FindoraError.cancelled }
        ...
        let temporary = file.url.deletingLastPathComponent().appending(
            path: ".findora-ocr-\(UUID().uuidString).pdf"
        )
    ...
    private func validateOutput(_ url: URL, expectedPages: Int) throws -> [ExtractedPage] {
        guard fileManager.fileExists(atPath: url.path),
            ...
            throw FindoraError.invalidPDF("OCR-Ausgabedatei fehlt.")
        }
        defer { try? handle.close() }
        let signature = try handle.read(upToCount: 5)
        guard signature == Data("%PDF-".utf8) else {
            throw FindoraError.invalidPDF("Ungültige PDF-Signatur.")
        }
        ...
        guard extractor.hasUsableTextLayer(pages) else {
            throw FindoraError.invalidPDF("Keine brauchbare Textschicht nach OCR.")
        }
        if let pdfInfo = dependencies.pdfInfo {
            ...
            guard integrity.status == 0 else {
                throw FindoraError.invalidPDF("pdfinfo meldet einen Strukturfehler.")
            }
        }
        return pages
    }
