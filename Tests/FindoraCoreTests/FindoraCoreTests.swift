...
func memoryPressureHandlerHopsFromBackgroundQueueToMainActor() async throws {
    ...
    let sourceQueue = DispatchQueue(
        label: "de.findora.tests.memory-pressure-source",
        qos: .utility
    )
    let callbackQueue = DispatchQueue(
        label: "de.findora.tests.memory-pressure-callback",
        qos: .utility
    )
...
func appFileLoggerCreatesReadablePrivateLog() async throws {
    ...
    #expect(fileURL.lastPathComponent == "Findora.log")
...
func scannerFindsNestedPDFsAndSkipsSymlinks() async throws {
    let root = FileManager.default.temporaryDirectory
        .appending(path: "FindoraScanner-\(UUID().uuidString)", directoryHint: .isDirectory)
...
func realMLXEmbeddingModelCanDownloadValidateAndRunWhenRequested() async throws {
    ...
    guard ProcessInfo.processInfo.environment["FINDORA_RUN_MODEL_TESTS"] == "1"
...
private func temporaryTestDirectory() -> URL {
    let url = FileManager.default.temporaryDirectory
        .appending(path: "FindoraTests-\(UUID().uuidString)", directoryHint: .isDirectory)
...
private func createTextPDF(at url: URL, pages: [String]) throws {
    ...
    guard let context = CGContext(url as CFURL, mediaBox: &mediaBox, nil) else {
        throw FindoraError.invalidPDF("Test-PDF-Kontext konnte nicht angelegt werden.")
    }
    for text in pages {
...
private func createImagePDF(at url: URL, text: String) throws {
    ...
    guard let page = PDFPage(image: image) else {
        throw FindoraError.invalidPDF("Synthetische Bildseite konnte nicht erstellt werden.")
    }
    let document = PDFDocument()
    document.insert(page, at: 0)
    guard document.write(to: url) else {
        throw FindoraError.invalidPDF("Synthetische Scan-PDF konnte nicht geschrieben werden.")
    }
}
