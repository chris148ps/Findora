...
private func maintenanceTemporaryDirectory() -> URL {
    let url = FileManager.default.temporaryDirectory
        .appending(path: "FindoraMaintenanceTests-\(UUID().uuidString)")
...
private func createLegacyMaintenanceDatabase(at url: URL) throws {
    ...
    guard sqlite3_open(url.path, &connection) == SQLITE_OK,
          let connection else {
        throw FindoraError.database("Legacy-Testdatenbank konnte nicht geöffnet werden.")
    }
...
private func readLegacyMigrationRows(at url: URL) throws -> [String: String] {
    ...
    guard sqlite3_open_v2(url.path, &connection, SQLITE_OPEN_READONLY, nil) == SQLITE_OK,
          let connection else {
        throw FindoraError.database("Migrierte Testdatenbank konnte nicht gelesen werden.")
    }
    defer { sqlite3_close(connection) }
    ...
    let statement else {
        throw FindoraError.database(String(cString: sqlite3_errmsg(connection)))
    }
...
private func createMaintenanceImagePDF(at url: URL) throws {
    ...
    guard let page = PDFPage(image: image) else {
        throw FindoraError.invalidPDF("Bildseite konnte nicht erstellt werden.")
    }
    let document = PDFDocument()
    document.insert(page, at: 0)
    guard document.write(to: url) else {
        throw FindoraError.invalidPDF("Bild-PDF konnte nicht geschrieben werden.")
    }
}
