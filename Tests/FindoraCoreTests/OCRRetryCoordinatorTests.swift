...
private actor ScriptedOCRProcessor: OCRProcessing {
    ...
    func process(
        ...
        guard let response = responses[strategyID] else {
            throw FindoraError.processFailed("Fehlende Teststrategie")
        }
        switch response {
        case .failure:
            throw FindoraError.processFailed("Synthetischer OCR-Fehler")
        case .delayed:
            try await Task.sleep(for: delay)
            throw FindoraError.processFailed("Unerwartetes Testende")
